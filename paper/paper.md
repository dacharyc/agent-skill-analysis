...
- A behavioral evaluation of 19 representative skills demonstrating that structural contamination scores do not predict behavioral degradation (r = 0.077), identifying five distinct content interference mechanisms, and finding that realistic agentic context mitigates roughly two-thirds of measured degradation
...
Examining the degradation patterns across all 19 skills reveals that cross-language code confusion — the mechanism our structural scoring was designed to detect — is only one of several interference vectors. We identified five distinct mechanisms:
...
3. **Token budget competition**: Under fixed output token limits, skill-influenced outputs allocate disproportionate tokens to commentary at the expense of code. In the react-native case, with-skill outputs spend 2.6× more tokens on explanatory prose and 18% fewer on code compared to baseline, producing less complete implementations.
...
These mechanisms have different implications for skill authors. Template propagation and API hallucination are addressable through content review (fix invalid syntax in templates, avoid suggestive API patterns). Textual frame leakage and token budget competition are harder to mitigate because they emerge from the skill's identity and scope rather than specific content defects.
